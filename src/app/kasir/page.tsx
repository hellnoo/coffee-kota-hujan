...
function buildDailyReport(orders: Order[], date: string, shifts: Shift[] = [], outletLabel = 'Cafe'): string {
  ...
  const lines = [
    ...
    ``,
    `_Laporan otomatis dari Kota Hujan POS_ 🚀`,
...
function showBrowserNotif(title: string, body: string) {
  ...
  if (Notification.permission === 'granted') {
    new Notification(title, { body, icon: '/favicon.svg', tag: 'kotahujan-order' })
  }
}
...
function msgSiap(order: Order) {
  ...
  return `Halo ${order.customer_name || 'Kak'}! 👋\n\nPesananmu di *Coffee Kota Hujan* sudah siap diambil. 🔔\n\n🪑 ${meja}\n💳 Pembayaran: ${bayar}\n\nSilakan ke kasir ya! ☕`
}
...
function msgStruk(order: Order) {
  ...
  return [
    ...
    `Sampai jumpa lagi di Kota Hujan 🤎`,
    ``,
    `_Struk digital — Kota Hujan POS_`,
  ].join('\n')
}
...
export default function KasirPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (res.ok) {
      localStorage.setItem('kotahujan-kasir', 'ok')
      setAuthed(true)
      ...
      <header className="bg-h-dark border-b border-h-border">
        <div className="max-w-4xl mx-auto px-5 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 flex-wrap justify-end">
            ...
            <button onClick={() => { localStorage.removeItem('kotahujan-kasir'); setAuthed(false) }}
              ...
              <div className="overflow-y-auto flex-1 p-5">
                <div className="bg-white text-black rounded-lg p-5 font-mono text-xs leading-relaxed shadow-lg">
                  <div className="text-center mb-2">
                    <div className="font-black text-sm tracking-wider">KOTA HUJAN</div>
