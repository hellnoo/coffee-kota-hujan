  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="400" height="200" viewBox="0 0 400 200">
    <defs>
      <linearGradient id="g" x1="0%" y1="0%" x2="100%" y2="100%">
        <stop offset="0%" stop-color="#EE7220"/>
        <stop offset="100%" stop-color="#2B1505"/>
      </linearGradient>
    ...
    <text x="200" y="178" text-anchor="middle" font-family="system-ui,sans-serif" font-size="10" fill="rgba(246,193,147,0.45)" letter-spacing="5">KOTA HUJAN</text>
  </svg>`
...
function ChatbotWidget({ items }: { items: MenuItem[] }) {
  ...
  const [messages, setMessages] = useState<{ role: 'user' | 'assistant'; content: string }[]>([
    { role: 'assistant', content: 'Halo Kak! ☕ Aku barista AI Kota Hujan. Mau rekomendasi menu? Tanya aja, atau pilih cepat di bawah 👇' }
  ])
...
const ORDER_KEY = (table: string) => `kotahujan-order-${table}`
...
function MenuContent() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem(`kotahujan-cart-${tableNum}`)
    if (saved) try { setCart(JSON.parse(saved)) } catch { /* ignore */ }
  }, [tableNum])
  useEffect(() => {
    if (Object.keys(cart).length > 0) localStorage.setItem(`kotahujan-cart-${tableNum}`, JSON.stringify(cart))
    else localStorage.removeItem(`kotahujan-cart-${tableNum}`)
  }, [cart, tableNum])
  ...
  if (restoring) return (
    <div className="min-h-screen bg-h-bg flex items-center justify-center">
      <div className="text-center">
        <div className="font-sans font-black text-white tracking-widest text-xl uppercase mb-2">KOTA HUJAN</div>
      ...
      <div className="min-h-screen bg-h-bg flex flex-col items-center justify-center text-center px-8">
        {isCancelled ? (
          ...
          <>
            ...
            <p className="text-h-muted text-xs mt-3 max-w-xs">Terima kasih sudah mampir ke Kota Hujan ☕</p>
            {/* Rating */}
            {!rated ? (
        ...
        <div className="mt-8 flex flex-col items-center gap-3">
          ...
          {(isCancelled || isDone) && (
            <button
              onClick={() => { setCart({}); setNote(''); setCustomerName(''); setPhone(''); setPayMethod(''); setSubmitted(false); setOrderId(null); clearActiveOrder(tableNum); localStorage.removeItem(`kotahujan-cart-${tableNum}`) }}
